...
func SetVcapRequestIdHeader(request *http.Request, logger zap.Logger) {
	guid, err := uuid.GenerateUUID()
	if err == nil {
		request.Header.Set(VcapRequestIdHeader, guid)
		if logger != nil {
			logger.Debug("vcap-request-id-header-set", zap.Object(VcapRequestIdHeader, guid))
		}
	}
}
...
func SetB3Headers(request *http.Request, logger zap.Logger) {
	...
	if err != nil {
		logger.Error("failed-to-create-b3-trace-id", zap.Error(err))
		return
	}
...
func setB3SpanIdHeader(request *http.Request, logger zap.Logger) {
	randBytes, err := secure.RandomBytes(8)
	if err != nil {
