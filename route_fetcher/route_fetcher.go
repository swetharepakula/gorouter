...
func NewRouteFetcher(logger zap.Logger, uaaClient uaa_client.Client, routeRegistry registry.RegistryInterface,
	...
	for {
		...
		case <-signals:
			...
			if es := r.eventSource.Load(); es != nil {
				...
				if err != nil {
					r.logger.Error("Failed to close routing_api EventSource: ", zap.Error(err))
				}
			}
			ticker.Stop()
			return nil
...
func (r *RouteFetcher) subscribeToEvents(token *schema.Token) error {
	...
	if err != nil {
		...
		r.logger.Error("Failed-to-subscribe-to-routing-api-event-stream: ", zap.Error(err))
		return err
	}
	...
	for {
		...
		if err != nil {
			metrics.IncrementCounter(SubscribeEventsErrors)
			r.logger.Error("Failed to get next event: ", zap.Error(err))

			closeErr := source.Close()
			if closeErr != nil {
				r.logger.Error("failed-to-close-event-source", zap.Error(closeErr))
			}
			break
		}
