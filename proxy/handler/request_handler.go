...
func NewRequestHandler(request *http.Request, response utils.ProxyResponseWriter, r reporter.ProxyReporter, alr *schema.AccessLogRecord, logger zap.Logger) *RequestHandler {
...
func setupLogger(request *http.Request, logger zap.Logger) zap.Logger {
	return logger.With(
		zap.String("session", "request-handler"),
...
func (h *RequestHandler) Logger() zap.Logger {
	return h.logger
}
...
func (h *RequestHandler) HandleWebSocketRequest(iter route.EndpointIterator) {
	h.logger.Info("handling-websocket-request, Upgrade: websocket")
